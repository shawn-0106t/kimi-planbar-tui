...
/** A 200 JSON Response factory shared by the fetchQuota test groups. (Was
 *  scoped to the taxonomy describe; the bearer describe referenced it out of
 *  scope — fetchQuota swallowed the ReferenceError after the header capture,
...
describe("the bearer header is built the way SPEC 16.1 says", () => {
  test("Authorization and Accept headers, GET only", async () => {
    ...
    // `seen` is assigned inside the fetchImpl closure, which TS's flow
    // analysis cannot track; the cast expresses the real type at this point.
