...
// model (SPEC 12). No terminal imports: this returns data rows as values, and
...
// the Min(0) spacer and the bottom-pinned footer are app.ts's job.
...
function titleLine(p: Palette, width: number, quota: QuotaResult | null): TuiLine {
  ...
  // ratatui Layout[Min(10), Length(16)]: Length wins, so the right column is
  // 16 cells whenever it fits at all; the title gets what remains.
