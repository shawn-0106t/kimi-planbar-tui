...
async function spawnKimiVersion(): Promise<{ code: number | null; output: string }> {
  ...
  try {
    const [code, out, err] = await Promise.race([
      Promise.all([exited, collect(proc.stdout!), collect(proc.stderr!)]),
      guard,
    ]);
    const lossy = (bytes: Buffer): string =>
 ...
 *  `Accept-Encoding: identity` is kept as a defense against a gzipped 206
 *  partial response (SPEC §22.4). */
