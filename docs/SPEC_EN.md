...
- `docs/` — this specification (`SPEC.md` / `SPEC_EN.md`), shared by the Rust and TS editions; **the mechanisms where a TS edition is not equivalent to the Rust edition are registered in chapter 22**, filed by this document's chapter numbers
...
The **TS edition (Bun)** (prerequisites: Windows + Bun ≥ 1.3, no Cargo involved; installed on this machine via `npm i -g --allow-scripts=bun bun` — the plain `npm i -g bun` is blocked by the npm allowScripts policy and skips Bun's postinstall):
...
> This chapter is the register of every mechanism where a TS edition — the Bun edition (`ts/`, Bun + OpenTUI) or the Node edition (`ts-nodejs/`, Node 24 + handwritten ANSI) — is not mechanically equivalent to the Rust edition, filed by this document's chapter numbers. The behavior contract in the preceding chapters is benchmarked on the Rust edition; every entry here is an implementation-level equivalence or a known deviation — the Rust/Node-related entries are pinned by tests, while the Bun-edition entries evolve with the code again since the 2026-09-25 unfreeze (entries recorded during the freeze period are kept as they were written).
>
> **Status**: the Bun edition (`ts/`) was frozen from 2026-09-19 to 2026-09-25 and is maintained again; Bun-edition entries in this chapter are maintained against the current code, and freeze-period records are kept with their measurement dates. The current implementations are the Rust edition and both TS editions.
...
| `resp.json()` failure | a strict in-house JSON parser (`parseJsonValue`): `1e400` raises `number out of range` → `JsonException`; **only the Bun edition** drains a non-2xx body via `arrayBuffer()` before returning (Bun returns a keep-alive connection to the pool only once the body is consumed; the Node edition deliberately does not drain, and reqwest drops the connection — a mechanism difference, identical output); a body timeout after the headers is also a `JsonException` | quota tests |
...
- **Mouse tracking must be explicitly disabled**: OpenTUI 0.5.x's `createCliRenderer` defaults `useMouse = true` and then emits `ESC[?1000h ?1002h ?1003h ?1006h` at startup — the mouse is hijacked entirely by the app (no select/copy in a real terminal), and the `?1003` any-motion event stream bypasses the `ensureRawMode()` re-assertion paths, so together with Bun flipping the console back to cooked on every stdin read, VT mouse sequences can echo onto the screen. This edition is a read-only keyboard dashboard (SPEC 12.7 defines no mouse at all), so `createTuiRenderer` must pass `useMouse: false` (fixed 2026-09-25; earlier releases carried the defect).
- **Restore handlers registered before terminal init** (matching the Node edition and the Rust panic-hook discipline): the `uncaughtException`/`unhandledRejection`/SIGINT/SIGBREAK restore closures must be installed **before** `await makeRenderer()`, tolerant of an absent renderer and raw-mode restore (so a `createCliRenderer` throw mid-setup still has a way out). `ts/src/tui/app.ts` implements this as of 2026-09-25; earlier releases registered them after init.
...
- **raw mode difference (important)**: Bun's `process.stdin.setRawMode()` does not change the OS console mode on Windows, and Bun flips the console back to cooked on every stdin read. The countermeasure: `ts/src/tui/console.ts` calls `SetConsoleMode` through `bun:ffi` (clearing `ENABLE_PROCESSED_INPUT|ENABLE_LINE_INPUT|ENABLE_ECHO_INPUT`, setting `ENABLE_WINDOW_INPUT`; the `PROCESSED_INPUT` clear exists only for crossterm parity — whether Ctrl+C is delivered is decided by the Bun runtime itself, see 22.6) and re-asserts raw mode before every input event and redraw; exit paths restore the original mode. Note the input handle is `(HANDLE)-10` = `0xfffffff6` (`0xfffffff5` is the output handle — M2 had this wrong); `bun:ffi` returns `bigint` for `i64`, so normalize with `Number()` before comparing.
- **The `sanitize()` injection firewall matches the Node edition**: OpenTUI's StyledText does not filter control characters, so every external string (skill names/descriptions, API copy) passes through `sanitize()` in `ts/src/tui/line.ts` before entering a frame — whole ANSI sequences (CSI/OSC/ESC+char) stripped first, then leftover C0/DEL/C1 controls, the same rules and order as the Node edition's `ansi.ts`. The choke point is the render-entry `padLineToWidth`, which every `TuiLine` passes through; the tests include injection cases (`ts/test/line.test.ts`).
...
- **Ctrl+C delivery on the Bun edition drifts with the Bun version (first measured 2026-09-20, re-measured 2026-09-25)**: `ts/src/tui/console.ts` clears `ENABLE_PROCESSED_INPUT` for crossterm parity (which also keeps Ctrl+B/Ctrl+H arriving as bytes), but whether Ctrl+C reaches the app is decided by the Bun runtime — **the 2026-09-20 measurement (Bun version of that date) had it fully swallowed** (neither a keypress nor a JS SIGINT; a bare bun process ignored Ctrl+C, the Rust edition quit from the same window under the same injected keystroke, and OpenTUI's parser did turn `0x03` into `{name:"c",ctrl:true}`, see `ts/scripts/verify/parse-ctrlc.ts`); **re-measured 2026-09-25 on Bun 1.4.2 / WT 1.24, Ctrl+C now quits cleanly** (graceful exit, EXITCODE=0, terminal fully restored), so the old record no longer holds. `q` remains the primary quit key; the SIGINT/SIGBREAK handlers and the `ctrl+c` router branch in `app.ts` stay (both paths work). Reproduction scripts: `ts/scripts/verify/`; on an untested Bun version, assume neither path.
- **The Bun-edition router ignores every Ctrl combination (except the Ctrl+C quit)**: `handleKey` returns early on `key.ctrl` after the `q`/Ctrl+C check and before the per-view dispatch, so e.g. Ctrl+R never refreshes; the Rust edition's key dispatch looks only at `KeyCode`, never at modifiers (a `Char('r')` carrying CONTROL still hits the refresh arm — `rust/src/app.rs:236-243`). A deliberate, known deviation, pinned by `ts/test/appRouting.test.ts`.
