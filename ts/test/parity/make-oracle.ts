...
//
//   bun run test/parity/make-oracle.ts        # writes test/golden/*.txt
// Requires: cargo with the crate deps already fetched. Not part of the app.
import { mkdirSync, readFileSync, writeFileSync } from "node:fs";
import { join } from "node:path";

const REPO = join(import.meta.dir, "..", "..", "..");
const OUT = process.env["KPT_ORACLE_DIR"] ?? join(process.env["TEMP"] ?? "/tmp", `kpt-oracle-${Math.random().toString(36).slice(2)}`);
...
console.log(
  [
    ...
    `  cp ${OUT.replaceAll("\\", "/")}/golden/*.txt ${join(REPO, "ts", "test", "golden").replaceAll("\\", "/")}/`,
